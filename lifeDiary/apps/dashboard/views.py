...
def dashboard_view(request):
    ...
    user_tags = Tag.objects.filter(Q(user=request.user) | Q(is_default=True)).order_by('-is_default', 'name')
...
def time_block_api(request):
    ...
    try:
        ...
        if not selected_date:
            return error_response('올바른 날짜 형식(YYYY-MM-DD)을 사용해주세요.', 'INVALID_DATE_FORMAT')

    except json.JSONDecodeError:
        return error_response('올바른 JSON 형식이 아닙니다.', 'INVALID_JSON')

    if request.method == 'POST':
        return _handle_time_block_create_update(request, data, slot_indexes, selected_date)
...
def _handle_time_block_create_update(request, data, slot_indexes, selected_date):
    ...
    try:
        ...
        return error_response(f'저장 중 오류가 발생했습니다: {str(e)}', 'SERVER_ERROR', 500)
    ...
    try:
        ...
        return success_response(
            f'{deleted_count}개의 슬롯이 삭제되었습니다.',
            {
                ...
                'requested_count': len(slot_indexes)
            }
        )
        
    except Exception as e:
        return error_response(f'삭제 중 오류가 발생했습니다: {str(e)}', 'SERVER_ERROR', 500)
