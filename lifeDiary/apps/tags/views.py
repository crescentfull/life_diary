...
def tag_list_create(request):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            return JsonResponse({'success': False, 'message': f'태그 조회 중 오류: {e}'}, status=500)
        ...
        try:
            ...
            if is_default and not request.user.is_superuser:
                return JsonResponse({'success': False, 'message': '기본 태그는 관리자만 생성할 수 있습니다.'}, status=403)
            
            if not name or not color:
                return JsonResponse({'success': False, 'message': '태그명과 색상을 입력해주세요.'}, status=400)

            # 중복 확인
            if Tag.objects.filter(Q(user=request.user, name=name) | Q(is_default=True, name=name)).exists():
                 return JsonResponse({'success': False, 'message': '이미 같은 이름의 태그가 존재합니다.'}, status=400)
            ...
            return JsonResponse({
                ...
                'tag': {'id': tag.id, 'name': tag.name, 'color': tag.color, 'is_default': tag.is_default}
            }, status=201)
            
        except json.JSONDecodeError:
            return JsonResponse({'success': False, 'message': '잘못된 형식의 요청입니다.'}, status=400)
        except Exception as e:
            return JsonResponse({'success': False, 'message': f'태그 생성 중 오류: {e}'}, status=500)
...
def tag_detail_update_delete(request, tag_id):
    ...
    if request.method == 'PUT':
        ...
        try:
            ...
            if is_default and not request.user.is_superuser:
                return JsonResponse({'success': False, 'message': '기본 태그는 관리자만 설정할 수 있습니다.'}, status=403)

            if not name or not color:
                return JsonResponse({'success': False, 'message': '태그명과 색상을 입력해주세요.'}, status=400)

            # 중복 확인 (자신 제외)
            if Tag.objects.filter(Q(user=request.user, name=name) | Q(is_default=True, name=name)).exclude(id=tag.id).exists():
                 return JsonResponse({'success': False, 'message': '이미 같은 이름의 태그가 존재합니다.'}, status=400)
            ...
            return JsonResponse({'success': False, 'message': f'태그 수정 중 오류: {e}'}, status=500)
        ...
        try:
            ...
            return JsonResponse({'success': True, 'message': f'"{tag_name}" 태그가 삭제되었습니다.'})
        
        except Exception as e:
            return JsonResponse({'success': False, 'message': f'태그 삭제 중 오류: {e}'}, status=500)
